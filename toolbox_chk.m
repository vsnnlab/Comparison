...
if ~isempty(Index_matlab)
    ...
    release_chk_matlab = release_yr_matlab >= 2021;
else
    release_chk_matlab = false;
end

if isempty(Index_matlab) ||  ~release_chk_matlab
    warning('MATLAB must be upgraded to version R2021a or above')
end
...
if ~isempty(Index_vision)
    ...
    release_chk_vision = release_yr_vision >= 2021;
else
...
if isempty(Index_vision) ||  ~release_chk_vision
    warning('Deep Learning Toolbox must be installed or upgraded to version R2021a or above')
    warning('Step 1: Go to the Home tab.')
    warning('Step 2: Click add-on icon.')
    warning('Step 3: Search and download Deep Learning Toolbox.')
end
...
if isempty(Index_vision2) 
    ...
    warning('Step 1: Go to the Home tab.')
    warning('Step 2: Click add-on icon.')
    warning('Step 3: Search and download Deep Learning Toolbox Model for AlexNet Network.')
end
