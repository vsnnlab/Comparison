        ...
        if any(tempAct(:))
            ...
            if ind == 1
                ...
                for cond_i = 1:size(totData,1)
                    ...
                    [~,idx_min] = min(mean(tempAct_cond,1));
                    if isequal(idx_max,idx_min); chk = chk+1; end
                end
...
for type_i = 1 : 2
    ...
    figure('Position',[100 + 300*(type_i-1) 500 300 200]); hold on;
    for ii = 1:2
        ...
        for cond_i = 1:3
            ...
            [~,ind] = max(tp(cond_i,:));
            h = plot([1 1]*xx(cond_i,ind),[0 1]*12,'--');
            h.Color = col(cond_i);
        end
